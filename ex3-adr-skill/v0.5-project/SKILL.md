...
1. Найди каталог ADR. Используй `adr/` по умолчанию; предпочитай существующий `docs/adr`, `doc/adr` или `architecture/decisions`.
2. Получи номер командой `python scripts/next-adr-number.py <adr_dir>`. Не выбирай номер вручную.
3. Сформируй slug из решения в Latin kebab-case.
4. Запиши файл как `<adr_dir>/adr-NNNN-{slug}.md`.
5. Укажи фактический путь к ADR в `trace.md`.
