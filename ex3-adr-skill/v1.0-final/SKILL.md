...
Перед завершением исправь типичные ошибки:

- Выдуманная альтернатива → заменить на `—` и добавить задачу.
- Выдуманный trade-off или цель → заменить на `—` и добавить задачу.
- `чтобы` повторяет решение → исправить по источнику; если данных нет, поставить `—`.
- `принимая` содержит преимущество → исправить по источнику; если данных нет, поставить `—`.
- Есть `—`, но нет задачи → добавить задачу.
- Нет `—`, но есть секция задач → удалить секцию.
- Два предложения, лишние секции или неверный порядок → исправить.
- Номер выбран вручную → получить номер скриптом.
